...
Features:
- Improved word-furigana-type analysis with complex pattern handling
- Fixed post-processing for お茶 (hiragana before kanji) and 引っ越したい (mixed patterns)
...
class EnhancedGrammaticalAnalyzer:
    """Enhanced grammatical analyzer with improved post-processing for complex patterns"""
    ...
    def __init__(self, config: BurnerConfig):
        ...
        try:
            ...
            print("✅ Enhanced grammatical analyzer initialized")
    ...
    def analyze_sentence(self, text: str, lang_hint: Optional[str] = None) -> List[GrammaticalWord]:
        """Analyze Japanese sentence with enhanced post-processing"""
        ...
        try:
            # Enhanced prompt with examples for complex cases
...
- お茶 → {{"word": "お茶", "furigana": "おちゃ", "type": "noun"}}
- 引っ越したい → {{"word": "引っ越したい", "furigana": "ひっこしたい", "type": "verb"}}
- 学生 → {{"word": "学生", "furigana": "がくせい", "type": "noun"}}
- です → {{"word": "です", "furigana": "です", "type": "auxiliary"}}

CRITICAL: Always provide furigana for every word, even hiragana ones. For compound words with mixed kanji/hiragana, provide complete furigana."""

            # Handle force refresh by changing cache filename
            if self.config.force_refresh:
                # Use timestamp to force fresh request but still cache result
                import time
                cache_key = f"enhanced_{abs(hash(text))}_{int(time.time())}.json"
            else:
                # Normal cache filename
                cache_key = f"enhanced_{abs(hash(text))}.json"
            ...
            result = self.openai_client.send_request_with_json_schema(
                ...
                filename=cache_key,
                schema_name="enhanced_analysis",
                model=self.config.openai_model
            )
            
            if result and 'words' in result:
                return self._process_enhanced_result(result['words'])
            else:
                return self._create_fallback_words(text)
    ...
    def _process_enhanced_result(self, words_data: List[Dict]) -> List[GrammaticalWord]:
        """Process result with enhanced post-processing for complex patterns"""
        ...
        for word_data in words_data:
            ...
            # ENHANCED POST-PROCESSING: Handle complex patterns
            processed_parts = self._enhanced_post_process(word, furigana, grammatical_type, color)
            processed_words.extend(processed_parts)
        
        furigana_count = sum(1 for w in processed_words if w.furigana)
        print(f"✅ Enhanced Analysis: {len(processed_words)} words, {furigana_count} with furigana")
        
        return processed_words
    
    def _enhanced_post_process(self, word: str, furigana: str, 
                             grammatical_type: GrammaticalType, 
                             color: Tuple[int, int, int]) -> List[GrammaticalWord]:
        """Enhanced post-processing for complex Japanese patterns"""
        
        if not furigana:
            return [GrammaticalWord(word=word, furigana=None, grammatical_type=grammatical_type, color=color)]
        
        # Case 1: Handle お茶 pattern (hiragana prefix + kanji)
        if self._is_okurigana_prefix_pattern(word, furigana):
            return self._handle_prefix_pattern(word, furigana, grammatical_type, color)
        
        # Case 2: Handle 引っ越したい pattern (complex mixed pattern)
        if self._is_complex_mixed_pattern(word, furigana):
            return self._handle_complex_mixed_pattern(word, furigana, grammatical_type, color)
        
        # Case 3: Handle standard okurigana suffix (聞こえて pattern)
        if self._should_split_suffix(word, furigana):
            return self._handle_suffix_pattern(word, furigana, grammatical_type, color)
        
        # Case 4: Simple cleanup
        cleaned_furigana = self._clean_simple_furigana(word, furigana)
    ...
    def _is_okurigana_prefix_pattern(self, word: str, furigana: str) -> bool:
        """Check for お茶 pattern: hiragana prefix + kanji"""
        if len(word) < 2:
            return False
            
        # Check if starts with hiragana and has kanji
        if (self._is_hiragana(word[0]) and 
            any(self._is_kanji(c) for c in word[1:]) and
            len(furigana) > len(word)):
            return True
    ...
    def _handle_prefix_pattern(self, word: str, furigana: str, 
        ...
        if kanji_start > 0:
            # Split: hiragana prefix (no furigana) + kanji part (with remaining furigana)
            prefix_part = word[:kanji_start]
            kanji_part = word[kanji_start:]
            
            # Calculate furigana for kanji part (remove prefix from furigana)
            kanji_furigana = furigana[kanji_start:] if kanji_start < len(furigana) else furigana
            
            # Only add furigana to kanji part if it's different from the word
    ...
    def _is_complex_mixed_pattern(self, word: str, furigana: str) -> bool:
        """Check for 引っ越したい pattern: complex kanji-hiragana mix"""
        if len(word) < 3:
            return False
            
        # Check for pattern: kanji-hiragana-kanji-hiragana or similar complex patterns
        char_types = []
        for char in word:
            if self._is_kanji(char):
                char_types.append('K')
        ...
        complex_patterns = ['KHKH', 'KHHH', 'KHKH', 'KHHKH', 'KHHHH']
        
        return any(p in pattern for p in complex_patterns)
    
    def _handle_complex_mixed_pattern(self, word: str, furigana: str, 
                                    grammatical_type: GrammaticalType, 
                                    color: Tuple[int, int, int]) -> List[GrammaticalWord]:
        """Handle 引っ越したい pattern by intelligent splitting"""
        
        # Strategy: Find kanji segments and assign furigana accordingly
        segments = []
        current_segment = ""
        current_furigana = ""
        furigana_pos = 0
        ...
        for i, char in enumerate(word):
            ...
            if self._is_kanji(char):
                # Try to find corresponding furigana
                # This is a simplified approach - in reality would need more sophisticated matching
                if furigana_pos < len(furigana):
                    # Estimate furigana length for this kanji (typically 1-3 characters)
                    estimated_len = min(3, len(furigana) - furigana_pos)
                    current_furigana += furigana[furigana_pos:furigana_pos + estimated_len]
                    furigana_pos += estimated_len
            
            elif self._is_hiragana(char):
                # For hiragana, check if it matches the furigana
                if furigana_pos < len(furigana) and furigana[furigana_pos] == char:
                    furigana_pos += 1
                
                # If we have accumulated kanji + hiragana, consider ending segment
                if len(current_segment) > 1 and i < len(word) - 1:
                    # Look ahead - if next char is kanji, end this segment
                    if self._is_kanji(word[i + 1]):
                        segments.append((current_segment, current_furigana if current_furigana != current_segment else None))
                        current_segment = ""
                        current_furigana = ""
        ...
        # If no good segmentation, fall back to suffix removal
        if len(segments) <= 1:
            return self._handle_suffix_pattern(word, furigana, grammatical_type, color)
        
        print(f"   COMPLEX: {word} → {segments}")
        
        # Create GrammaticalWords with same color
        result = []
        for segment_word, segment_furigana in segments:
            result.append(GrammaticalWord(
                word=segment_word,
                furigana=segment_furigana,
                grammatical_type=grammatical_type,
                color=color
            ))
    ...
    def _should_split_suffix(self, word: str, furigana: str) -> bool:
        """Check if word should be split due to okurigana suffix"""
        if len(word) < 2:
            return False
            
        # Check for kanji followed by hiragana pattern
        has_kanji = any(self._is_kanji(c) for c in word)
        has_hiragana = any(self._is_hiragana(c) for c in word)
        
        if not (has_kanji and has_hiragana):
            return False
            
        # Check if furigana is significantly shorter than expected for all chars
        return len(furigana) < len(word) * 1.5
    
    def _handle_suffix_pattern(self, word: str, furigana: str, 
                             grammatical_type: GrammaticalType, 
                             color: Tuple[int, int, int]) -> List[GrammaticalWord]:
        """Handle 聞こえて pattern: kanji + hiragana suffix"""
        
        # Find the split point (last kanji position)
        last_kanji_pos = -1
        for i in range(len(word) - 1, -1, -1):
            if self._is_kanji(word[i]):
                last_kanji_pos = i
                break
        
        if last_kanji_pos >= 0 and last_kanji_pos < len(word) - 1:
            kanji_part = word[:last_kanji_pos + 1]
            hiragana_suffix = word[last_kanji_pos + 1:]
            
            # Clean furigana by removing matching suffix
            cleaned_furigana = self._remove_matching_suffix(kanji_part, furigana, hiragana_suffix)
            
            print(f"   SUFFIX: {word} → {kanji_part}({cleaned_furigana}) + {hiragana_suffix}")
    ...
    def _clean_simple_furigana(self, word: str, furigana: str) -> Optional[str]:
        """Simple furigana cleanup for cases that don't need splitting"""
    ...
    def _remove_matching_suffix(self, kanji_part: str, furigana: str, hiragana_suffix: str) -> Optional[str]:
        """Remove matching hiragana suffix from furigana"""
        
        if not hiragana_suffix or not furigana:
            return furigana
        
        # If furigana ends with the hiragana suffix, remove it
        if furigana.endswith(hiragana_suffix):
            cleaned = furigana[:-len(hiragana_suffix)] if len(hiragana_suffix) < len(furigana) else None
...
class EnhancedFuriganaBurnerApp:
    ...
    def process_video_with_subtitles(self, video_path: str, subtitle_path: str, output_path: str = None) -> bool:
        ...
        if not output_path:
            output_path = os.path.join(video_dir, f"{video_name}_enhanced.mp4")
        ...
        try:
            ...
            json_output = os.path.join(video_dir, f"{subtitle_name}_enhanced.json")
            srt_output = os.path.join(video_dir, f"{subtitle_name}_enhanced.srt")
    ...
    if len(sys.argv) == 1:
        ...
        if video_files and subtitle_files:
            ...
            if success:
                ...
                print("   ✅ Case A: お茶 → お + 茶(ちゃ) [Fixed prefix handling]")
                print("   ✅ Case B: 聞こえて → 聞(き) + こえて [Fixed suffix handling]") 
                print("   ✅ Case C: 引っ越したい → Intelligently segmented [Fixed complex patterns]")
                print("   ✅ Color consistency across split words")
                print("   ✅ Output saved to video directory")
            else:
                print("\n❌ FAILED!")
        else:
