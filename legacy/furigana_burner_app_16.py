...
- Improved non-Japanese subtitle support  
...
class ImprovedGrammaticalAnalyzer:
    """Improved grammatical analyzer with conservative suffix-based post-processing"""
    ...
    def __init__(self, config: BurnerConfig):
        ...
        try:
            ...
            print("✅ Improved grammatical analyzer initialized")
    ...
    def analyze_sentence(self, text: str, lang_hint: Optional[str] = None) -> List[GrammaticalWord]:
        """Analyze sentence with improved handling for both Japanese and non-Japanese text"""
        ...
        # If no Japanese characters or explicitly marked as non-Japanese, use fallback
        if not has_japanese or (lang_hint and lang_hint.lower() not in ['ja', 'japanese', 'jpn']):
            return self._create_enhanced_fallback_words(text, is_japanese=False)
        
        # Try AI analysis for Japanese text
        if not self.openai_client:
            return self._create_enhanced_fallback_words(text, is_japanese=True)
        
        try:
            # Improved prompt with emphasis on accurate furigana
...
CRITICAL: Provide complete, accurate furigana for the entire word. """
            ...
            if self.config.force_refresh:
                ...
                import time
                cache_key = f"improved_{abs(hash(text))}_{int(time.time())}.json"
            else:
                # Normal cache filename
                cache_key = f"improved_{abs(hash(text))}.json"
            ...
            result = self.openai_client.send_request_with_json_schema(
                ...
                filename=cache_key,
                schema_name="improved_analysis",
                model=self.config.openai_model
            )
            
            if result and 'words' in result:
                return self._process_improved_result(result['words'])
            else:
                return self._create_enhanced_fallback_words(text, is_japanese=True)
    ...
    def _process_improved_result(self, words_data: List[Dict]) -> List[GrammaticalWord]:
        ...
        for word_data in words_data:
            ...
            # IMPROVED POST-PROCESSING: Conservative suffix-based approach
            processed_parts = self._improved_post_process(word, furigana, grammatical_type, color)
            processed_words.extend(processed_parts)
        
        furigana_count = sum(1 for w in processed_words if w.furigana)
        print(f"✅ Improved Analysis: {len(processed_words)} words, {furigana_count} with furigana")
        
        return processed_words
    
    def _improved_post_process(self, word: str, furigana: str, 
                             grammatical_type: GrammaticalType, 
                             color: Tuple[int, int, int]) -> List[GrammaticalWord]:
        """Improved conservative post-processing focusing on suffix patterns"""
    ...
    def _create_enhanced_fallback_words(self, text: str, is_japanese: bool = False) -> List[GrammaticalWord]:
        """Create enhanced fallback words for both Japanese and non-Japanese text"""
        words = []
        current_word = ""
        
        # Choose appropriate default color based on language
        default_color = GrammaticalColorScheme.get_color(GrammaticalType.OTHER)
        if not is_japanese:
            # Use a more neutral white color for non-Japanese text
        ...
        for char in text:
            if char in separators:
                ...
                # Add punctuation as separate word if it's not just whitespace
                if char.strip():
                    punctuation_color = GrammaticalColorScheme.get_color(GrammaticalType.PUNCTUATION)
                    if not is_japanese:
                        punctuation_color = (200, 200, 200)  # Light gray for non-Japanese punctuation
        ...
        language_type = "Japanese" if is_japanese else "Non-Japanese"
        print(f"✅ {language_type} Fallback: {len(words)} words created")
...
class ImprovedFuriganaBurnerApp:
    """Improved furigana burner with audio preservation and enhanced non-Japanese support"""
    
    def __init__(self, config: BurnerConfig = None):
        self.config = config or BurnerConfig()
        self.analyzer = ImprovedGrammaticalAnalyzer(self.config)
        ...
        print(f"🚀 Improved Furigana Burner initialized ({cache_status}, Audio: {audio_status})")
    ...
    def process_video_with_subtitles(self, video_path: str, subtitle_path: str, output_path: str = None) -> bool:
        ...
        if not output_path:
            output_path = os.path.join(video_dir, f"{video_name}_improved.mp4")
        ...
        try:
            ...
            json_output = os.path.join(video_dir, f"{subtitle_name}_improved.json")
            srt_output = os.path.join(video_dir, f"{subtitle_name}_improved.srt")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Improved Grammatical Furigana Burner with Audio Support')
    ...
    parser.add_argument('--no-cache', action='store_true', 
                       help='Force fresh fetch (ignore cache but still save newly fetched cache)')
    ...
    if len(sys.argv) == 1:
        ...
        if video_files and subtitle_files:
            config = BurnerConfig()
            app = ImprovedFuriganaBurnerApp(config)
            success = app.process_video_with_subtitles(video_files[0], subtitle_files[0])
            
            if success:
                print("\n🎉 SUCCESS! Improved furigana processing completed!")
                ...
                print("   ✅ Non-Japanese subtitles → Enhanced fallback rendering")
