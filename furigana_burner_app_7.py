...
class HybridLineFuriganaGenerator:
    ...
    def get_line_furigana(self, text: str, lang_hint: Optional[str] = None) -> List[FuriganaChar]:
        """Generate line-level furigana using hybrid approach"""
        ...
        try:
            # Step 1: Pykakasi preprocessing
            pykakasi_result = self._get_pykakasi_preprocessing(text)
            
            # Step 2: AI refinement
            refined_readings = self._get_ai_refinement(text, pykakasi_result)
            
            # Step 3: Create character info with colors
            chars = self._create_furigana_chars(text, refined_readings)
            
            readings_count = len([c for c in chars if c.furigana])
            print(f"✅ Hybrid furigana: {text[:30]}... ({readings_count} characters with readings)")
            return chars
