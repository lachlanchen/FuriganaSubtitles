#!/usr/bin/env python3
"""
Modular Furigana Subtitle Burner App
Highly reusable with pluggable furigana generation
Handles multi-line subtitles and proper positioning
"""
...
class BurnerConfig:
    ...
    line_spacing: float = 1.4  # Spacing between multiple lines
    stroke_width: int = 2
...
class FuriganaRenderer:
    """Renders furigana text with proper multi-line support"""
    ...
    def split_into_lines(self, furigana_chars: List[FuriganaChar], max_width: int) -> List[SubtitleLine]:
        """Split furigana text into multiple lines if needed"""
        ...
        for char_info in furigana_chars:
            ...
            if (current_width + char_width > max_width and 
                ...
                if self._is_good_break_point(char_info.char) or len(current_line) > 10:
                    line_width, line_height = self._measure_line(current_line)
                    lines.append(SubtitleLine(current_line, line_width, line_height))
                    current_line = []
                    current_width = 0
        ...
        return lines or [SubtitleLine(furigana_chars, max_width, self.config.main_font_size * 2)]
    ...
    def _measure_line(self, furigana_chars: List[FuriganaChar]) -> Tuple[int, int]:
        """Measure the total size of a line"""
        total_width = sum(self._measure_char_width(char) for char in furigana_chars)
        
        # Height includes furigana space
        height = self.config.main_font_size
        if any(char.furigana for char in furigana_chars):
            height += self.config.furigana_font_size + int(self.config.furigana_font_size * 0.3)
    ...
    def render_multi_line_subtitle(self, lines: List[SubtitleLine], 
        ...
        """Render multi-line subtitle with proper positioning"""
        if not lines:
        ...
        if len(lines) > 1:
            total_height += int((len(lines) - 1) * self.config.line_spacing * self.config.main_font_size)
        ...
        for line in lines:
            ...
            y_offset += line.height + int(self.config.line_spacing * self.config.main_font_size)
        
        return img
    
    def _render_single_line(self, draw: ImageDraw.Draw, line: SubtitleLine, 
                           total_width: int, y_offset: int):
        """Render a single line of furigana text"""
        ...
        for char_info in line.furigana_chars:
            ...
            char_y = y_offset + self.config.furigana_font_size + int(self.config.furigana_font_size * 0.3)
            
            # Draw character with stroke
            self._draw_text_with_stroke(draw, char_x, char_y, char_info.char, self.main_font)
...
class FuriganaBurnerApp:
    """Main furigana subtitle burner application"""
    ...
    def __init__(self, config: BurnerConfig = None, furigana_generator: FuriganaGenerator = None):
        ...
        self.renderer = FuriganaRenderer(self.config)
        
        print(f"🚀 Furigana Burner App initialized")
    ...
    def _add_subtitle_to_frame(self, frame: np.ndarray, text: str, 
        ...
        """Add subtitle to frame with proper positioning"""
        ...
        if (x + subtitle_width > frame_width or 
            ...
            print(f"⚠️  Subtitle position unsafe: pos=({x},{y}), size=({subtitle_width},{subtitle_height}), frame=({frame_width},{frame_height})")
            return frame
        ...
        try:
            ...
            if overlay_region.shape == subtitle_rgb.shape == alpha.shape:
                ...
                print(f"⚠️  Shape mismatch: {overlay_region.shape} vs {subtitle_rgb.shape}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Furigana Subtitle Burner App')
    ...
    if len(sys.argv) == 1:
        ...
        if mp4_files and srt_files:
            ...
            app = FuriganaBurnerApp(config, generator)
            ...
            print("\n📖 Usage:")
            print("  python furigana_burner_app.py                    # Auto-detect files")
            print("  python furigana_burner_app.py video.mp4 sub.srt  # Specify files")
            print("  python furigana_burner_app.py --position top     # Top position")
